...
- **Density altitude** — `DA = PA + 120 * (OAT - ISA_temp)`, used in cruise TAS and RPM lookups. Cruise tables (`cruiseTAS.best_power` / `cruiseTAS.economy`, `fullThrottleTAS`) are digitized from POH Fig. 5-19/5-21/5-23 and keyed by DA in 1000 ft steps; `cruiseRPM` only has the 0 and 12 000 ft rows since its lines are straight. `maxPowerAvailable(DA)` clamps requested power to the full-throttle limit (75% to ~7500 ft DA, 65% at 12 000 ft).
